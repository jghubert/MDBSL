...
/* 
 * File:   FastSim_Phototaxis_Compass.cpp
...
namespace MDB_Social {
    ...
    FastSim_Phototaxis_Compass::FastSim_Phototaxis_Compass(std::string id) 
    :GAFitness(id)
    {
        registerParameters();
...
//        world->getMap()->add_illuminated_switch(light);
    ...
    FastSim_Phototaxis_Compass::FastSim_Phototaxis_Compass(const FastSim_Phototaxis_Compass& orig) 
    {
        
    }

    FastSim_Phototaxis_Compass::~FastSim_Phototaxis_Compass() 
    {
        delete world;
        delete controller;
        delete babbling;
    ...
    void FastSim_Phototaxis_Compass::registerParameters()
    {
        std::cout << "FastSim_Phototaxis_Compass : registering the parameters...";
        std::cout.flush();
//        Settings* settings = Settings::getInstance();
    ...
    void FastSim_Phototaxis_Compass::loadParameters()
    {
        std::cout << "FastSim_Phototaxis_Compass: Loading parameters..." << std::endl;
//        Settings* settings = Settings::getInstance();
        try {
...
//            testIndividual = settings->value<bool>("General.testIndividual").second;
            ...
            world->initialize();
            std::cout << "FastSim_Phototaxis_Compass: Parameters loaded." << std::endl;
        }
        catch (std::exception e) {
            std::cerr << "FastSim_Phototaxis_Compass: Error loading the parameters: " << e.what() << std::endl;
            exit(1);
        }
...
#else
            std::cerr << "FastSim_Phototaxis_Compass: the REV viewer is not compiled in." << std::endl;
#endif            
        }
    ...
    void FastSim_Phototaxis_Compass::installGenotype(Genotype& individual)
    {
        ...
        if (weights.size() != individual.getSize()) {
            std::cerr << "FastSim_Phototaxis_Compass: Size of the genotype (size = " << individual.getSize() << ") differs from the size of the network (size = " << weights.size() << ")." << std::endl;
            exit(1);
        }
    ...
    void FastSim_Phototaxis_Compass::logRobotPosition(unsigned trial, unsigned epoch)
    {
        if (logRobotPos) {
    ...
    void FastSim_Phototaxis_Compass::relocateRobot()
    {
    ...
    bool FastSim_Phototaxis_Compass::computeReward()
    {
...
//        std::cout << "Phototaxis : dist = " << dist << std::endl;
    ...
    double FastSim_Phototaxis_Compass::computeDistance()
    {
        // Compute the distance between the goal and the robot, and compute the reward accordingly.
        double x = world->getRobot()->get_pos().get_x();
        double y = world->getRobot()->get_pos().get_y();
...
//        std::cout << "Phototaxis : dist = " << dist << std::endl;
    ...
    FastSim_Phototaxis_Compass::compass_info_t FastSim_Phototaxis_Compass::computeCompass()
    {
        compass_info_t ret;
        double x = world->getRobot()->get_pos().get_x();
        double y = world->getRobot()->get_pos().get_y();
...
//            ret.orientation += 2*M_PI;
    ...
    double FastSim_Phototaxis_Compass::evaluateFitness(Genotype& individual, unsigned gen, unsigned ind, bool _testIndividual)
    {
...
//        }
        ...
        if (showFastSimViewer) {
            if (!world->activateViewer(true))
                std::cout << Color::Modifier(Color::FG_RED) << "FastSim_Phototaxis_Compass: ERROR: fastsim viewer not compiled in. Activate it using cmake -DUSE_FASTSIM_VIEWER=ON" << Color::Modifier(Color::FG_DEFAULT) << std::endl;
            else {
                std::cout << "FastSim_Phototaxis_Compass: Viewer activated." << std::endl;
            }
        }
...
//        std::cout << "Phototaxis: rewardTotal = " << rewardTotal << " ; fitness = " << (rewardTotal*1.0)/(1.0*trialCount*epochCount) << std::endl;
        ...
        if (fitnessComparisonTest) {
            ...
            if (!fcfile.is_open()) {
                std::cerr << "FastSim_Phototaxis_Compass: Error opening the log file fitnessComparisonTest.log." << std::endl;
                exit(1);
            }
    ...
    void FastSim_Phototaxis_Compass::testValueFunction()
    {
...
//                    nninputs[nbinputs-1] = 1.0;   // bias
    ...
    void FastSim_Phototaxis_Compass::testCompass()
    {
